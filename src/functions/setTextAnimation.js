...
const calculateTextWidth_1 = __importDefault(require("../utilities/calculateTextWidth"));
function setTextAnimation(text, textBoxDimensions, x, y, fontFamily, lineHeight, shadow) {
    ...
    const calculateXPos = (alignment, lineWidth, cumulativeWidth) => {
        let baseXPos = x + (maxWidth - lineWidth) / 2;
        if (alignment === 'left')
            return x + cumulativeWidth;
        if (alignment === 'right')
            return x + (maxWidth - lineWidth) + cumulativeWidth;
        return baseXPos + cumulativeWidth;
    };
    //CALCOLA LA COORDINATA Y DI PARTENZA DAL QUALE INIZIARE A MOSTRARE IL TESTO
    const calculateYPos = (lineIndex) => Math.round(y + (lineIndex * lineHeight));
    text.animations.forEach((animation) => {
            ...
            if (animation.split === "word") {
                ...
                lines.forEach((line, lineIndex) => {
                    ...
                    words.forEach((word, wordIndex) => {
                        ...
                        const wordWidth = (0, calculateTextWidth_1.default)(word, fontSize);
...
exports.default = setTextAnimation;
