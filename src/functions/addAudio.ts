import { Audio } from "../model/model";
import parsePercentage from "../utilities/parsePercentage";

function addAudio(audio: Audio, index: any): string[] {
  // Initialize filter strings
  let volumeFilter = "";
  let fadeFilter = "";
  let trimFilter = "";
  let delayFilter = "";
  let duckingFilters: string[] = [];
  ...
  if (audio.duckings) {
    audio.duckings.forEach(({ start, end, volume }) => {
      ...
      duckingFilters.push(
        `,volume=enable='between(t,${start},${end || start + 3})':volume=${parsePercentage(volume) || 0.2}`
      );
    });
  }
  ...
  if (audio.volume) {
    volumeFilter = `,volume=${parsePercentage(audio.volume) || 1}`;
  }
