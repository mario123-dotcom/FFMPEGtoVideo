...
function replaceSingleQuote(text: string): string {
    return text.replace(/'/g, '’');
}

function textIntoLines(textOptions: Text): { fontSize: number, lines: string[], maxWidth: number, totalHeight: number, lineWidths: number[] } {
    ...
    function calculateLineHeight(fontSize: number, leading: number): number {
        return fontSize + leading;
    }
    
    function calcolaRigheTesto(fontSize: number): { lines: string[], maxWidth: number, lineWidths: number[] } {
    ...
    const { lines, maxWidth, lineWidths } = calcolaRigheTesto(fontSize);
    const altezzaTesto = calculateLineHeight(fontSize, sizeToPixels(textOptions.line_height, textOptions.height));
